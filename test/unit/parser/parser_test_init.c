...
struct option options[] = {
    ...
    [FLAG_TRACE_LEXIS] = {.long_name="trace-lexis",
                    .flags=GOPT_ARGUMENT_FORBIDDEN},
    [FLAG_TRACE_SYNTAXIS] = {.long_name="trace-syntaxis",
                    .flags=GOPT_ARGUMENT_FORBIDDEN},
...
void set_options(char *test, struct option options[])
{
    ...
    if (options[FLAG_DEBUG].count) {
        debug_opamc_test     = options[FLAG_DEBUG].count;
    }
    if (options[FLAG_DEBUG_LEXIS].count) {
        debug_opamc_lexis    = options[FLAG_DEBUG_LEXIS].count;
    }
    if (options[FLAG_DEBUG_SYNTAXIS].count) {
        debug_opamc_syntaxis = options[FLAG_DEBUG_SYNTAXIS].count;
    }
    if (options[FLAG_TRACE].count) {
        trace_opamc_test      = true;
    }
    if (options[FLAG_TRACE_LEXIS].count) {
        trace_opamc_lexis     = true;
    }
    if (options[FLAG_TRACE_SYNTAXIS].count) {
